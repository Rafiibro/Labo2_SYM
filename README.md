...
Pour le code d'erreur, on vérifie si la réponse retourne le code 200 (HTTP_OK) avec le test

 ```if (responseCode == HttpURLConnection.HTTP_OK)```, si oui, il n'a pas d'erreur.
...
### Question 4.4 Écriture différée
...
Si les requêtes sont lourdes, les multiplexer seraient une mauvaise idée. Donc la connexion différée est une meilleure solution.

Si des requêtes passent mais d'autres non à cause de problèmes de connexion, il n'y a que celle qui n’ont pas été transmise qui sont gardée dans un buffer.

Les requêtes sont plus facile à sérialiser et à compresser.

##### Désavantages:

Il n'est pas possible de l'utiliser si le serveur demande une authentification.
...
Comme toute les requêtes sont "fusionnées" en une seule, il faut un moyen de les différencier entre elles.

Il faudrait donc un champs ID que le serveur applicatif puisse reconnaître pour les traiter correctement.
...
JSON et XML sont "human readable". En comparaison Protocol Buffers n'est pas censé être "human readable".

Protocol Buffers est plus pratique pour modéliser des structure de données.

Protocol Buffers offre une vérification de la structure de données. Qui est très polyvalente, par exemple, des balises comme required, optional, et repeated sont disponible.
...
Protocol Buffers un processing des données plus rapide.
...
L'api devrait fournir plus de méthode pour permettre une plus grande agilité dans la façon de l'utiliser. Il y a aussi un problème majeur. Si le nom d'un champ change il faudra modifier la requête alors que si il y a des méthode de type get il ne faudrait pas forcément modifier le code.

Lors d'une utilisation mobile il serait bien de pouvoir avoir les mises à jour qui aurait été effectuée dans la BDD grâce à une requête pour pouvoir mettre a jour l'interface. Le problème est qu'il faudrait faire des requête périodiquement pour être sur de ne rien louper.
